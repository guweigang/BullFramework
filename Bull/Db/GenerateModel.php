...
class Bull_Db_GenerateModel
{
    ...
    {
        $this->name = $name;     
        $this->sql = Bull_Db::getConnect($name);
    }
    ...
    protected function code($tbl, $model)
    {
        ...
        $root_dir   = Bull_Registry::get('config')->system->directory. "/";
        $model_dir  = Bull_Registry::get('config')->model->directory;
        ...
        $code = <<<EOT
<?php
class {$class_prefix}_{$model_prefix}_{$model_name} extends Bull_Db_AbstractModel
{
     protected \$table = "{$tbl}";
