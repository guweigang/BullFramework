...
abstract class Bull_Db_AbstractModel extends Bull_Singleton
{
    /**
     *
     * 数据库配置
    ...
    {
        $sql = Bull_Db::getConnect($this->name);
    ...
    {
        /* 强制切到主库操作 */
        Bull_Db::setConf($this->name);
        $sql = Bull_Db::connect("master");
     ...
     *
     * @return mixed (object Bull_Element | bool)
     *
     */
    public function validate($data = null)
    {
        /* 初始化过滤器 */
        $filter = new Bull_Db_Filter(new Bull_Element());
        /* 初始化元素及规则 */
        ...
        if ($data === null)
        {
            ...
            /* 异常 */
            throw new Bull_Db_Exception(Bull_Locale::get("ERR_ILLEGAL_PARAMS"));
        }
    }
}